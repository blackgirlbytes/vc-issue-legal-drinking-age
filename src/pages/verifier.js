...
const VerifierPage = () => {
    ...
    const [isPresentationDefinitionValid, setIsPresentationDefinitionValid] = useState(false);
    const [isPresentationExchangeSatisfied, setIsPresentationExchangeSatisfied] = useState(false);
    ...
    const displayDID = isDIDExpanded ? verifierDid.did : `${verifierDid.did?.substring(0, 20)}...`;
    const toggleDIDView = () => {
    ...
    const verifyCredential = async () => {
        try {
                ...
                'input_descriptors': [
                    {
                        ...
                        'constraints': {
                            'fields': [
                                {
                                    'path': [
                                        '$.credentialSubject.isOfLegalDrinkingAge',
                                    ]
                                }
                            ]
                        }
            ...
            PresentationExchange.satisfiesPresentationDefinition({ vcJwts: [jwt], presentationDefinition });
    ...
    return (
        <div style={styles.container}>
            ...
            />
            <button style={styles.button} onClick={verifyCredential}>Verify Credential</button>
            {isVerificationSuccessful && (
                <div style={styles.resultsContainer}>
                    <p>Verification successful: {isVerificationSuccessful.toString()}</p>
                    <p>Presentation definition valid: {isPresentationDefinitionValid.toString()}</p>
                    <p>Presentation exchange satisfied: {isPresentationExchangeSatisfied.toString()}</p>
                </div>
            )}
        </div>
    );
