...
const SubjectPage = () => {
      ...
      if (jwt) {
        try {
          ...
          setCredentialDetails({
            ...
            legalDrinkAge:
              parsedVc.vcDataModel.credentialSubject.isOfLegalDrinkingAge,
          });
        } catch (error) {
  ...
  const handleShareToTwitter = async () => {
    ...
    try {
      const response = await axios.post(
        "https://api.cloudinary.com/v1_1/dd8qwbjtv/image/upload",
        formData
      );
  ...
  return (
    <div style={styles.container}>
      ...
      <p
        onClick={toggleDIDView}
        style={{ ...styles.clickable, ...styles.label }}
      >
        DID: {displayDID}
      </p>
      <button style={styles.button} onClick={handleCopyDid}>
        Copy DID
      </button>
      <button style={styles.button} onClick={handleCopyJwt}>
        Copy JWT
      </button>
      ...
      {jwt && (
        <div>
          <div id="print" style={styles.credentialCardStyles}>
            ...
            <p style={styles.credentialDetail}>
              {credentialDetails.legalDrinkAge ? "Yes" : "No"}
            </p>
          ...
          <button style={styles.button} onClick={handleShareToTwitter}>
            Share to Twitter
          </button>
